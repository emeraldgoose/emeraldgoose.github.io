...
모델링부터 모델 배포까지 해볼 수 있었던 프로젝트입니다. MLflow나 데이터 라벨링 문제가 있었지만 다른 방법으로 해결할 수 있었습니다.  

파이프라인 설계를 위해 airflow까지 해보고 싶었는데 SQLAlchemy하고 충돌때문인지 `airflow db init` 를 실행하면 자꾸 에러가 발생했고 다운그레이드로도 해결하지 못했습니다.

그리고 MLflow의 파이프라인 작성을 해보지 못해서 이와 관련된 airflow와 mlflow의 파이프라인 작성을 연습해보려고 합니다.

마지막으로 Docker를 제대로 써본건 이번이 처음인데 정말 편한 도구였습니다. 
