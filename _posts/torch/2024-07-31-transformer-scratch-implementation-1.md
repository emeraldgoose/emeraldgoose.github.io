...
왼쪽 그래프에서 하나가 1.0이지만 오른쪽 그래프에서는 0.4x로 줄고 나머지 정보들이 드러나는 것을 볼 수 있습니다. 즉, softmax로 정보가 사라져 기울기도 사라지는 gradient vanishing 문제를 스케일링으로 해결할 수 있습니다. 또한, $d_k$를 사용하는 이유는 다음과 같습니다.

Query와 Key가 가우시안 분포를 따른다고 가정하면 평균은 0, 분산은 1을 가지게 됩니다.  
dot-product를 수행하면 $\sum_{i}^{d_k}Q_iK_i$이고 Query와 Key는 independent하므로 평균은 0, 분산은 $d_k$를 가지게 됩니다.  
따라서 $\sqrt{d_k}$로 스케일링하여 분산을 1로 줄일 수 있게 됩니다.
...
2번 수식에서 얻을 수 있는 $\frac{\partial W}{\partial A}$는 softmax의 미분결과이므로 W(1 - W)로 가정해보겠습니다.(실제 계산은 W(1 - W)가 아니고 저는 미리 구현해둔 softmax 클래스를 이용할 것입니다.)  
...
## MultiHead Attention
MultiHead Attention은 Attention을 여러개의 head가 동시에 수행하는 모듈입니다. 하나의 문장에 대해 관계 혹은 단어 등에 집중할 수 있는 어션을 생각해볼 수 있고 이러한 어텐션을 병렬로 처리하기 위해 구현되었습니다.
...
먼저, Loss를 $\hat{x}$에 대한 미분을 계산하겠습니다.

$\frac{\partial L}{\partial \hat{x}} = \gamma$

다음, Loss를 분산에 대한 미분을 계산하겠습니다.
...
다음, Loss를 평균에 대한 미분을 계산하겠습니다. 2번에서 분산을 계산할 때 평균이 참여하므로 분산을 평균에 대한 기울기도 추가되어야 합니다.
...
![](https://lh3.googleusercontent.com/d/1ZBaXBoaaqzIxz0ssOlas6LPykTWocuB3)
...
![](https://lh3.googleusercontent.com/d/1w8igm0PfyrZHyKPnrOh9VeIFRSva-pUm)
